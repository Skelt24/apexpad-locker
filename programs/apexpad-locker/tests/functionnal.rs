...
use solana_program::{hash::Hash, pubkey::Pubkey, rent::Rent, sysvar, system_program};
use solana_program_test::{processor, ProgramTest};
use solana_sdk::{account::Account, signature::Keypair, signature::Signer, system_instruction, transaction::Transaction};
use apexpad_locker::{entrypoint::process_instruction, instruction::Schedule};
...
#[tokio::test]
async fn test_apexpad_locker() {
    // Create program and test environment
    let program_id = Pubkey::from_str("AL7nyF8UR5MsmXrmnCf8SoAyT7ccK2DDQh6GZufbWcje").unwrap();
    ...
    // Add the extend instruction test
    let extend_instructions = [
        extend(
            ...
            &destination_token_account.pubkey(),
            &mint.pubkey(),
            vec![Schedule {amount: 20, release_time: 10}],
            seeds.clone()
        ).unwrap()
    ];

    // Process setup transaction on test network
    ...
    // Process vesting transactions on test network
    ...
    // Process change destination transaction on test network
    ...
    // Process extend schedule transaction on test network
    let mut extend_transaction = Transaction::new_with_payer(
        &extend_instructions, 
        Some(&payer.pubkey())
    );
    extend_transaction.partial_sign(
        &[
            &payer,
            &source_account
        ], 
