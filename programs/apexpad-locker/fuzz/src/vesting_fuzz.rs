use apexpad_locker::instruction;
...
use apexpad_locker::{instruction::{Schedule, VestingInstruction}, processor::Processor};
use apexpad_locker::instruction::{init, unlock, change_destination, create, extend};
use solana_sdk::{account::Account, instruction::InstructionError, transaction::TransactionError};

struct SolToolsLockerEnv {
...
struct FuzzInstruction {
    ...
    // This flag decides whether the instruction will be executed with inputs that should
    // not provoke any errors. (The accounts and contracts will be set up before if needed)
    correct_inputs: bool
}

/// Use u8 as an account id to simplify the address space and re-use accounts more often.
type AccountId = u8;
...
fn main() {
    ...
    let apexpad_locker_testenv = SolToolsLockerEnv {
        system_program_id: system_program::id(),
        sysvarclock_program_id: sysvar::clock::id(),
        rent_program_id: sysvar::rent::id(),
        token_program_id: spl_token::id(),
        vesting_program_id: Pubkey::from_str("AL7nyF8UR5MsmXrmnCf8SoAyT7ccK2DDQh6GZufbWcje").unwrap(),
    ...
    loop {
        fuzz!(|fuzz_instructions: Vec<FuzzInstruction>| {
            ...
            let mut program_test = ProgramTest::new(
                "apexpad_locker",
                apexpad_locker_testenv.vesting_program_id,
                processor!(Processor::process_instruction),
            );

            program_test.add_account(apexpad_locker_testenv.mint_authority.pubkey(), Account {
            ...
            rt.block_on(run_fuzz_instructions(&apexpad_locker_testenv, &mut test_state.banks_client, fuzz_instructions, &test_state.payer, test_state.last_blockhash));
        });
    }
}

async fn run_fuzz_instructions(
    apexpad_locker_testenv: &SolToolsLockerEnv,
    ...
    banks_client.process_transaction(transaction).await.unwrap_or_else(|e| {
        if let BanksClientError::TransactionError(te) = e {
            match te {
    ...
    if fuzz_instruction.correct_inputs {
        ...
        match fuzz_instruction {
            FuzzInstruction {
                ...
                return (vec![init(
                    &apexpad_locker_testenv.system_program_id,
                    &apexpad_locker_testenv.rent_program_id,
                    &apexpad_locker_testenv.vesting_program_id,
            ...
            FuzzInstruction {
                ...
                return (instructions_acc, vec![clone_keypair(mint_key),
                    clone_keypair(&apexpad_locker_testenv.mint_authority),
            ...
            FuzzInstruction {
                ...
                let unlock_instruction = unlock(
                    &apexpad_locker_testenv.vesting_program_id,
                    &apexpad_locker_testenv.token_program_id,
                    &apexpad_locker_testenv.sysvarclock_program_id,
                    &correct_vesting_account_key,
                    &correct_vesting_token_key,
                    destination_token_key,
                    correct_seeds
            ...
            FuzzInstruction {
                instruction: VestingInstruction::Extend { .. },
                ..
            } => {
                let mut instructions_acc = vec![init(
                    &apexpad_locker_testenv.system_program_id,
                ...
                instructions_acc.push(extend_instruction);
                return (instructions_acc, vec![
                    clone_keypair(mint_key),
                    clone_keypair(&apexpad_locker_testenv.mint_authority),
                    clone_keypair(source_token_account_owner_key),
...
fn create_fuzzinstruction(
    ...
    let mut instructions_acc = mint_init_instruction(
        &payer,
        &mint_key,
        &apexpad_locker_testenv.mint_authority
    );
    ...
    let setup_instruction = mint_to(
        ...
        &mint_key.pubkey(),
        &correct_source_token_account_key,
        &apexpad_locker_testenv.mint_authority.pubkey(),
        &[],
        source_amount
    ).unwrap();
