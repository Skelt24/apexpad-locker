...
impl Processor {
    ...
    pub fn process_extend(
        ...
        let state_header = VestingScheduleHeader::unpack(
            &vesting_account.data.borrow()[..VestingScheduleHeader::LEN],
        )?;

        if state_header.mint_address != *mint_address {
            msg!("Provided Token Mint address is invalid");
            return Err(ProgramError::InvalidArgument);
        }
        ...
        let mut total_amount: u64 = 0;
        let add_amount = token_amount;

        let date = schedules[0].release_time;
        let old_date = old_schedules[0].release_time;

        if old_date > date {
            msg!("The unlock date provided is prior to the one defined at locker creation");
            return Err(ProgramError::InvalidArgument);
            
        }

        for s in old_schedules.iter_mut() {
            s.release_time = date;
            total_amount += s.amount + add_amount;
            s.amount += add_amount;
        }
        ...
        if Account::unpack(&source_token_account.data.borrow())?.amount < add_amount {
            msg!("The source token account has insufficient funds.");
            return Err(ProgramError::InsufficientFunds);
        };
    ...
    pub fn process_instruction(
        ...
        match instruction {
            ...
            VestingInstruction::Extend {
                ...
                msg!("Instruction: Extend Schedule");
                Self::process_extend(
                    program_id,
                    accounts,
                    seeds,
